...
def train(model, device,
    ...
    loader_args = dict(batch_size=batch_size, pin_memory=True, collate_fn=xenium_collate_fn) # TODO: add num_workers back; cut out to work in ipython
    ...
    celltype_criterion = nn.CrossEntropyLoss(reduction='mean', weight=torch.Tensor(dataset.class_counts[:,2:].mean()/dataset.class_counts[:,2:].mean(axis=0))) # Class imbalance reweighting
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in train_loader:
            ...
            loss = foreground_criterion(foreground_pred[label_mask], (labels[label_mask]>0).type(torch.float))
            ...
            if nucleus_mask.sum() > 0:
                ...
                loss += celltype_criterion(class_pred[nucleus_mask], classes[nucleus_mask]-1)
