...
def estimate_cell_types(gene_counts,
                        min_components=2, max_components=25,
                        max_em_steps=100, tol=1e-4,
                        warm_start=False):
    ...
    final_expression_profiles, final_prior_probs, final_cell_types, final_expression_rates = [], [], [], []
    ...
    for idx, n_components in enumerate(range(min_components, max_components+1)):
        ...
        if warm_start and n_components > min_components:
            ...
            next_expression_profiles[-1] = cur_expression_profiles[dominant]*split_prob + (1-split_prob)*np.random.dirichlet(np.ones(n_genes))
            ...
            cur_expression_profiles = np.random.dirichlet(np.ones(n_genes), size=n_components)
        ...
        for step in range(max_em_steps):
            ...
            logits = (np.log(cur_prior_probs[None])
                        + (gene_counts[:,None] * np.log(cur_expression_profiles[None])).sum(axis=2))
            ...
            cur_expression_profiles = (cur_cell_types[...,None] * gene_counts[:,None]).sum(axis=0) + 1
            cur_expression_profiles = cur_expression_profiles / (cur_cell_types.sum(axis=0) + 1)[:,None]

            # M-step (part 2): estimate cell type probabilities
            cur_prior_probs = (cur_cell_types.sum(axis=0) + 1) / (cur_cell_types.shape[0]+n_components)
            print(cur_prior_probs)

            # Track convergence of the cell type profiles
            converge = np.linalg.norm(prev_expression_profiles - cur_expression_profiles) / np.linalg.norm(prev_expression_profiles)
        ...
        aic_scores[idx], bic_scores[idx] = aic_bic(gene_counts, cur_expression_profiles, cur_prior_probs)
...
def aic_bic(gene_counts, expression_profiles, prior_probs):
    ...
    log_likelihood = logsumexp(np.log(prior_probs[None]) + (gene_counts[:,None] * np.log(expression_profiles[None])).sum(axis=2), axis=1).sum()
...
def calculate_pixel_loglikes(tx_geo_df, tx_count_grid, expression_profiles, expression_rates):
    ...
    transcript_counts = np.zeros(tx_count_grid.shape + (max_transcript_count,), dtype=int)
    ...
    rate_loglike_uniques = poisson.logpmf(np.arange(max_count+1)[:,None], expression_rates[None])
    rate_loglikes = rate_loglike_uniques[totals.reshape(-1)].reshape(totals.shape + (expression_rates.shape[0],))
    ...
    for i in range(expression_loglikes.shape[0]):
        ...
        for j in range(expression_loglikes.shape[1]):
            expression_loglikes[i,j] = (transcript_counts[i,j,:,None] * log_expression.T[transcript_ids[i,j]]).sum(axis=0)
...
def load_and_filter_transcripts(transcripts_file: str, min_qv=20.0):
    ...
    tx_geo_df["gene_id"] = tx_geo_df["feature_name"].apply(
        lambda x: mapping.get(x, 0)
    )
...
def create_pixel_geodf(x_max, y_max):
    ...
    grid_df = pd.DataFrame(np.array(np.meshgrid(np.arange(x_max+1), np.arange(y_max+1))).T.reshape(-1,2), columns=['X','Y'])
...
def spatial_as_sparse_arrays(nuclei_file: str, transcripts_file: str,
                             out_dir: str,
                             pixel_stride=1, min_qv=20.0,
                             foreground_nucleus_distance=1,
                             background_nucleus_distance=10,
                             background_transcript_distance=4,
                             background_pixel_transcripts=5,
    ...
    x_max, y_max = int(tx_geo_df['x_location'].max()+1), int(tx_geo_df['y_location'].max()+1)
    ...
    labels_geo_df = gpd.sjoin_nearest(idx_geo_df, nuclei_geo_df, how='left', distance_col='nucleus_distance')
    labels_geo_df.rename(columns={'index_right': 'nucleus_id_xenium'}, inplace=True)

    # Calculate the nearest transcript neighbors
    from scipy.spatial import KDTree
    transcript_xy = np.array([tx_geo_df['x_location'].values, tx_geo_df['y_location'].values]).T
    ...
    labels_geo_df['transcript_distance'] = kdtree.query(pixels_xy, k=background_pixel_transcripts+1)[0][:,-1]
    ...
    background_pixels = ((labels_geo_df['nucleus_distance'] > background_nucleus_distance) &
    ...
    labels = np.zeros((labels_geo_df['X'].max()+1, labels_geo_df['Y'].max()+1), dtype=int)
    labels[labels_geo_df['X'], labels_geo_df['Y']] = pixel_labels

    # Create a nuclei x gene count matrix
    tx_nuclei_geo_df = gpd.sjoin_nearest(tx_geo_df, nuclei_geo_df, distance_col='nucleus_distance')
    nuclei_count_geo_df = tx_nuclei_geo_df[tx_nuclei_geo_df['nucleus_distance'] <= foreground_nucleus_distance]
    ...
    np.add.at(tx_count_grid, (tx_geo_df['x_location'].values.astype(int), tx_geo_df['y_location'].values.astype(int)), 1)
    ...
    tx_background_mask = labels[tx_geo_df['x_location'].values.astype(int), tx_geo_df['y_location'].values.astype(int)] == 0
    ...
    cell_type_rates = (cell_type_probs[L] * tx_count_grid[X, Y][:, None]).sum(axis=0) / cell_type_probs[L].sum(axis=0)
    ...
    for idx in range(cell_type_probs.shape[1]):
        i,j = idx // 3, idx % 3
        axarr[i,j].hist(cell_type_probs[np.argmax(cell_type_probs, axis=1)==idx,idx], bins=200)
    plt.show()

    # Combine the background and foreground classes
    all_expression_profiles = np.vstack([background_probs, results['expression_profiles'][best_k]])
    ...
    rate_loglikes, expression_loglikes = calculate_pixel_loglikes(tx_geo_df, tx_count_grid, all_expression_profiles, all_expression_rates)
    ...
    labels_geo_df['nucleus_angle'] = (
                                        (cart2pol(labels_geo_df['nucleus_centroid_x'].values - labels_geo_df['X'].values,
                                              labels_geo_df['nucleus_centroid_y'].values - labels_geo_df['Y'].values)[1] + np.pi)
                                        / (2*np.pi)
                                    )
    angles = np.zeros(labels.shape)
    angles[labels_geo_df['X'].values, labels_geo_df['Y'].values] = labels_geo_df['nucleus_angle'].values
    ...
    for x_start in np.arange(0,x_max+1,tile_stride):
        ...
        tx_local_x = tx_geo_df[tx_geo_df['x_location'].between(x_start, x_start+tile_width, inclusive='left')]
        ...
        for y_start in np.arange(0,y_max+1,tile_stride):
            ...
            # Filter transcripts and labels
            tx_local = tx_local_x[tx_local_x['y_location'].between(y_start, y_start+tile_height, inclusive='left')]
            ...
            labels_local = np.array(labels[x_start:x_start+tile_width, y_start:y_start+tile_height])
            ...
            angles_local = np.array(angles[x_start:x_start+tile_width, y_start:y_start+tile_height])
            ...
            classes_local = np.array(pixel_types[x_start:x_start+tile_width, y_start:y_start+tile_height])
