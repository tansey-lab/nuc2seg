...
def evaluate(net, dataloader, device, amp):
    ...
    for idx, batch in enumerate(dataloader):
        ...
        x, y, z, labels, label_mask = batch['X'], batch['Y'], batch['gene'], batch['labels'], batch['label_mask']
        ...
        for im_pred, im_true, im_label_mask in zip(foreground_pred, mask_true, label_mask):
            im_pred, im_true = im_pred[im_label_mask], im_true[im_label_mask]

            im_pred = (im_pred > 0.5).float()
            # compute the Dice score
            dice_score += dice_coeff(im_pred, im_true, reduce_batch_first=False) / mask_true.shape[0]
...
def plot_predictions(net, dataloader, idx=0, threshold=0.5):
    for i,batch in enumerate(dataloader):
        if i < idx:
            continue
        x, y, z, labels, angles, classes, label_mask, nucleus_mask = (batch['X'],
                                                                        batch['Y'],
                                                                        batch['gene'],
    ...
    for i,c in enumerate(np.random.choice(np.unique(label_plot[label_mask]), replace=False, size=int(label_plot[label_mask].max()+1))):
    ...
    fig, axarr = plt.subplots(3, x.shape[0],figsize=(5*x.shape[0],10), sharex=True, sharey=True)
    for i in range(x.shape[0]):
        ...
        axarr[2,i].imshow(foreground_pred[i], vmin=0, vmax=1, cmap='coolwarm', interpolation='none')
...
def score_segmentation(segments, nuclei):
    ...
    nuclei_uniques, inv_map, nuclei_counts = np.unique(nuclei[nuclei_mask], return_inverse=True, return_counts=True)
    ...
    for i in range(n_nuclei):
        ...
        local_uniques, local_counts = np.unique(local_labels[local_labels >= 0], return_counts=True)
    ...
    return { 'percent_foreground': percent_foreground,
