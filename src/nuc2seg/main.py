...
def sample_gp_fast(width, height, step=10, size=1, bandwidth=8, scale=3):
    ...
    XY_grid_small = np.array(np.meshgrid(np.arange(width, step=step), np.arange(height, step=step))).T.reshape(-1,2)
    Cov = np.array([(sqexp(x1[None], XY_grid_small, bandwidth=bandwidth, scale=scale, axis=1)) for x1 in XY_grid_small])
    ...
    XY_grid = np.array(np.meshgrid(np.arange(width), np.arange(height))).T.reshape(-1,2)
    ## scipy 1.7.0 version
...
def sample_poisson_points_fast(n_points, width, height):
    ...
    return XY_grid[np.random.choice(XY_grid.shape[0], size=n_points, p=probs, replace=False)]


def estimate_flow(Ligands, Receptors, ligands_per_receptor=1, lam=1, lam_min=1e-1, lam_max=500, n_lam=30, penalty_type='l1'):
    import torch
...
def visualize_flow(Ligands, Receptors, Grid, Flows):
    ...
    ligand_vectors = ((Receptors[None] - Ligands[:,None]) * Flows[...,None]).sum(axis=1)
    ...
    for i, ((y, x), (dy, dx)) in enumerate(zip(Grid, grid_vectors)):
        print(i)
        plt.arrow(x,y,dx*10,dy*10, width=0.2*np.sqrt(dx**2+dy**2), head_width=0.3*np.sqrt(dx**2+dy**2), color=(dx/2+0.5, dy/2+0.5, np.sqrt(dx**2+dy**2)))
...
def load_real_data(ligand_name = 'CXCL12', receptor_name='CXCR4', gene_name='VEGFA'):
    ...
    ligand_idxs = np.arange(df.shape[0])[(df['feature_name'] == ligand_name) & (df['qv'] >= 20)]
    receptor_idxs = np.arange(df.shape[0])[(df['feature_name'] == receptor_name) & (df['qv'] >= 20)]
    ...
    ligand_cells = np.random.choice(Ligands.shape[0], replace=False, size=Ligands.shape[0]//50)
    receptor_cells = np.random.choice(Receptors.shape[0], replace=False, size=Receptors.shape[0]//50)
    Ligands = Ligands[ligand_cells]
    Receptors = Receptors[receptor_cells]
    Genes = Genes[receptor_cells]

    return int(max(Ligands[:,1].max(), Receptors[:,1].max())+10), int(max(Ligands[:,0].max(), Receptors[:,0].max())+10), Ligands, Receptors, Genes
...
if __name__ == '__main__':
    ...
    for i, ((y, x), sinks) in enumerate(zip(Ligands, ligand_vectors)):
        ...
        for j, (dy, dx) in enumerate(sinks):
            if Flows[i,j] < min_prob:
                continue
            plt.arrow(x,y,dx*0.8,dy*0.8, width=2*Flows[i,j], head_width=2*Flows[i,j], color='black', alpha=Flows[i,j])
    plt.scatter(Ligands[:,1], Ligands[:,0], s=3, color='red', zorder=100, label='Ligands')
    plt.scatter(Receptors[:,1], Receptors[:,0], s=3, color='blue', zorder=100, label='Receptors')
    ...
    Grid = np.array(np.meshgrid(np.arange(width, step=max(10,width//100)), np.arange(height, step=max(10,height//100)))).T.reshape(-1,2)
