...
def calculate_neighbor_weights(transcript_xy, cell_xy, max_distance, nucleus_size, bandwidth):
...
def e_step(transcript_genes, transcript_neighbors, known_transcript_cells,
                local_weights, model_amplitudes, model_gene_probs, n_cells,
                init_cell_types=None,
                n_burn=100, n_thin=5, n_samples=100):
    # Initialize the cell assignments
    n_transcripts = transcript_genes.shape[0]
    cur_transcript_cells = np.array([z if z != -1 else np.random.choice(neighbors) 
    ...
    if init_cell_types is None:
        ...
        np.add.at(cell_transcript_counts, (cur_transcript_cells[cur_transcript_cells!=0]-1, transcript_genes[cur_transcript_cells!=0]), 1)
        cell_transcript_counts /= cell_transcript_counts.sum(axis=1, keepdims=True)
        cur_cell_types = np.argmin(np.linalg.norm(cell_transcript_counts[:,None] - model_gene_probs[None], axis=2), axis=1)+1
    else:
    ...
    transcript_cell_samples = np.zeros((n_samples, cur_transcript_cells.shape[0]), dtype=int)
    ...
    for step in range(n_burn + n_thin*n_samples):
        ...
        for i in range(n_transcripts):
            ...
            probs = calculate_transcript_cell_probs(cur_cell_types[transcript_neighbors[i]-1], local_weights[i], transcript_genes[i], model_gene_probs)
...
def fit_fused_lasso_count_process(counts, lam_min=1e-2, lam_max=50, n_lam=30, segmentation_threshold=0.1):
    import torch
    ...
    for lam_idx, lam in tqdm(enumerate(lams)):
        ...
        def loss(t_rates):
            ...
            if lam > 0:
                ...
                l += lam * (rows.sum() + cols.sum() + diag1.sum() + diag2.sum()) / (rows.shape[0] + cols.shape[0] + diag1.shape[0] + diag2.shape[0])

            return l

        # Optimize using a 2nd order method with autograd for gradient calculation.
        res = minimize(loss, cur_Rates, method='L-BFGS-B', backend='torch', bounds=(1e-5,None), tol=1e-6)
        cur_Rates = res.x
    ...
    dof = np.array([len(calc_plateaus((rates.reshape(-1) > rates.max()*segmentation_threshold).astype(int), edges, rel_tol=1e-2)) for rates in Rates])
    ...
    return Rates_final, Rates[::-1], lams[::-1] , dof[::-1], loglike[::-1], bic_scores[::-1]
...
def test_fused_lasso_count_process():
    ...
    Rates_final, Rates_hat, lams, dofs, loglikes, bic_scores = fit_fused_lasso_count_process(counts)
    ...
    for i, j in enumerate([0, 10, 22, 26, np.argmin(np.abs(dofs-2))]):
        axarr[i+1].imshow(Rates_hat[j] * (Rates_hat[j]>Rates_hat[j].max()*0.1), vmin=Rates_hat.min(), vmax=Rates_hat.max())
        axarr[i+1].set_title(f'lam={lams[j]:.2f} dof={dofs[j]}\nBIC={bic_scores[j]:.2f}')
    plt.show()
...
if __name__ == '__main__':
    ...
    transcript_neighbors, neighbor_weights, known_transcript_cells = calculate_neighbor_weights(transcript_xy, cell_xy, max_distance, nucleus_size, bandwidth)
    known_mask = known_transcript_cells != -1

    # Initialize transcript cell assignments just on distance
    nearest_cell = np.array([t[np.argmax(w)] for t, w in zip(transcript_neighbors, neighbor_weights)], dtype=int)
    cell_transcript_counts = np.zeros((n_cells, n_genes))
    np.add.at(cell_transcript_counts, (nearest_cell[known_transcript_cells!=0]-1, transcript_genes[known_transcript_cells!=0]), 1) 
    ...
    for step in range(max_steps):
        ...
        transcript_cell_samples, cell_type_samples = e_step(transcript_genes, transcript_neighbors, known_transcript_cells,
                                                            local_weights, model_amplitudes, model_gene_probs, n_cells,
        ...
        row_idx = np.repeat(np.arange(n_row), n_col).reshape(n_row, n_col)[foreground_mask]
        col_idx = np.repeat(np.arange(n_col), n_row).reshape(n_col, n_row).T[foreground_mask]
        np.add.at(cell_gene_counts, (row_idx, transcript_cell_samples[foreground_mask]-1, transcript_genes[col_idx]), 1)

        # Collapse further: group count vectors by the cell type
        count_vectors = [cell_gene_counts[cell_type_samples==k] for k in range(n_cell_types)]
    ...
    plt.scatter(cell_xy[:,0], cell_xy[:,1], c=cell_types, marker='*', s=200, zorder=100)
    plt.scatter(transcript_xy[:,0], transcript_xy[:,1], c=transcript_genes, marker='o', s=20)
    ...
    for i in range(n_transcripts):
        ...
        if mapped_cell == 0:
            if true_cell != 0:
                ...
                plt.arrow(x,y,true_dx,true_dy, width=0.1, head_width=0.05, color='blue')
            continue
