...
def simulate_gaussian_cells(img_width=100,img_height=100,
                            n_cell_types=6, n_genes=20, n_transcripts=1000,
                            nucleus_size=2, n_cells_approx=200, background_rate=0.05):
    ...
    for k in range(n_cell_types):
        ...
        locs = sample_poisson_points_fast(np.random.binomial(n_cells_approx, 0.5), img_width, img_height)
        cell_xy.extend(locs)
    ...
    for j in range(n_cells+1):
        if j == 0:
            ...
            transcript_xy[:offsets[j]] = np.random.random(size=(n_transcripts_per_cell[0],2))*np.array([img_width,img_height])[None]
            transcript_genes[:offsets[j]] = np.random.choice(n_genes, size=n_transcripts_per_cell[0])
        else:
            transcript_xy[offsets[j-1]:offsets[j]] = np.random.multivariate_normal(cell_xy[j-1], cell_covs[j-1], size=n_transcripts_per_cell[j])
            transcript_genes[offsets[j-1]:offsets[j]] = np.random.choice(n_genes, p=gene_probs[cell_types[j-1]-1], size=n_transcripts_per_cell[j])
            ...
            for i in range(offsets[j-1], offsets[j]):
                distances = np.sqrt(((transcript_xy[i:i+1] - cell_xy)**2).sum(axis=1))
                nearest = np.argmin(distances)
                while nearest != (j-1) and distances[nearest] < nucleus_size:
                    # Resample the x y location of this transcript
                    transcript_xy[i] = np.random.multivariate_normal(cell_xy[j-1], cell_covs[j-1])
                    distances = np.sqrt(((transcript_xy[i:i+1] - cell_xy)**2).sum(axis=1))
                    nearest = np.argmin(distances)
