...
def temp_forward(model, x, y, z):
    mask = z > -1
    b = torch.as_tensor(np.tile(np.arange(z.shape[0]), (z.shape[1],1)).T[mask.numpy().astype(bool)])
    W = model.filters(z[mask])
    t_input = torch.Tensor(np.zeros((z.shape[0],) + model.img_shape))
    t_input.index_put_((b, torch.LongTensor(x[mask]), torch.LongTensor(y[mask])), W, accumulate=True)
    t_input = torch.Tensor.permute(t_input,(0,3,1,2)) # Needs to be Batch x Channels x ImageX x ImageY
    return torch.Tensor.permute(model.unet(t_input), (0,2,3,1)) # Map back to Batch x ImageX x Image Y x Classes
...
def stitch_tile_predictions(model, dataset, tile_buffer=8):
    ...
    for idx in range(len(dataset)):
        ...
        x, y, z, labels, angles, classes, label_mask, nucleus_mask, location = (tile['X'],
                                                                    tile['Y'],
                                                                    tile['gene'],
        ...
        mask_pred = temp_forward(model, x[None], y[None], z[None]).squeeze(0).detach().numpy().copy() # TEMP: code is fixed but i am currently using a pretrained model
        ...
        print(x_start+x_start_offset,x_start+x_end_offset, y_start+y_start_offset,y_start+y_end_offset)
        results[x_start+x_start_offset:x_start+x_end_offset, y_start+y_start_offset:y_start+y_end_offset,0] = foreground_pred[x_start_offset:x_end_offset,y_start_offset:y_end_offset]
        results[x_start+x_start_offset:x_start+x_end_offset, y_start+y_start_offset:y_start+y_end_offset,1] = angles_pred[x_start_offset:x_end_offset,y_start_offset:y_end_offset]
        results[x_start+x_start_offset:x_start+x_end_offset, y_start+y_start_offset:y_start+y_end_offset,2:] = class_pred[x_start_offset:x_end_offset,y_start_offset:y_end_offset]

        idx += 1

    return results

def greedy_expansion(start_xy, pixel_labels_arr, flow_labels, flow_labels2, foreground_mask, max_expansion_steps = 50):
    for step in range(max_expansion_steps):
        ...
        update_mask = foreground_mask & (pixel_labels_flat == -1) & ((flow_labels_flat != -1) | (flow_labels_flat2 != -1))
        ...
        flow1_connected = (
                            (pixel_labels_arr[start_xy[flow1_mask,0]-1, start_xy[flow1_mask,1]-1] == flow1_targets) |
                            (pixel_labels_arr[start_xy[flow1_mask,0]+1, start_xy[flow1_mask,1]-1] == flow1_targets) |
                            (pixel_labels_arr[start_xy[flow1_mask,0]-1, start_xy[flow1_mask,1]+1] == flow1_targets) |
                            (pixel_labels_arr[start_xy[flow1_mask,0]+1, start_xy[flow1_mask,1]+1] == flow1_targets))
        flow1_targets[~flow1_connected] = -1
        pixel_labels_arr[start_xy[flow1_mask,0], start_xy[flow1_mask,1]] = flow1_targets#flow_labels_flat[flow1_mask]
        ...
        flow2_connected = (
                            (pixel_labels_arr[start_xy[flow2_mask,0]-1, start_xy[flow2_mask,1]-1] == flow2_targets) |
                            (pixel_labels_arr[start_xy[flow2_mask,0]+1, start_xy[flow2_mask,1]-1] == flow2_targets) |
                            (pixel_labels_arr[start_xy[flow2_mask,0]-1, start_xy[flow2_mask,1]+1] == flow2_targets) |
                            (pixel_labels_arr[start_xy[flow2_mask,0]+1, start_xy[flow2_mask,1]+1] == flow2_targets))
        flow2_targets[~flow2_connected] = -1
        pixel_labels_arr[start_xy[flow2_mask,0], start_xy[flow2_mask,1]] = flow2_targets
        ...
        island_mask[1:-1,1:-1] = ((pixel_labels_arr[1:-1,1:-1] == -1) & 
        ...
        flow_labels[start_xy[:,0], start_xy[:,1]] = np.maximum(pixel_labels_arr[flow_xy[:,0], flow_xy[:,1]], pixel_labels_arr[flow_xy2[:,0], flow_xy2[:,1]])
        flow_labels2[start_xy[:,0], start_xy[:,1]] = np.maximum(pixel_labels_arr[flow_xy[:,0], flow_xy[:,1]], pixel_labels_arr[flow_xy2[:,0], flow_xy2[:,1]])

def flow_destination(start_xy, angle_preds, magnitude):
    dxdy = np.array(pol2cart(magnitude, angle_preds))[:,start_xy[:,0],start_xy[:,1]].T
...
def flow_graph_segmentation(foreground_preds, angle_preds, class_preds,
                             nuclei_file = None,
    ...
    start_xy = np.array([labels_geo_df['X'].values, labels_geo_df['Y'].values], dtype=int).T
    ...
    if nuclei_file is not None:
        ...
        labels_geo_df = gpd.sjoin_nearest(labels_geo_df, nuclei_geo_df, how='left', distance_col='nucleus_distance')
...
def greedy_cell_segmentation(foreground_preds, angle_preds, class_preds,
                             nuclei_file = None,
                             foreground_nucleus_distance = 1,
                             foreground_threshold=0.5,
                             max_expansion_steps = 15,
    ...
    flow_labels[start_xy[:,0], start_xy[:,1]] = pixel_labels_arr[flow_xy[:,0], flow_xy[:,1]]

    flow_xy2 = flow_destination(start_xy, angle_preds, np.sqrt(3))
    flow_labels2 = np.zeros(img_shape, dtype=int) - 1
    flow_labels2[start_xy[:,0], start_xy[:,1]] = pixel_labels_arr[flow_xy2[:,0], flow_xy2[:,1]]
    
    # Get the pixels that are sufficiently predicted to be foreground
    foreground_mask = (foreground_preds >= foreground_threshold)[start_xy[:,0], start_xy[:,1]]
    ...
    if nuclei_file is not None:
        ...
        # Assign pixels roughly on top of nuclei to belong to that nuclei label
        nucleus_mask = labels_geo_df['nucleus_distance'] <= foreground_nucleus_distance
        pixel_labels_arr[labels_geo_df['X'].values[nucleus_mask], labels_geo_df['Y'].values[nucleus_mask]] = labels_geo_df['nucleus_label'][nucleus_mask]
        flow_labels[start_xy[:,0], start_xy[:,1]] = pixel_labels_arr[flow_xy[:,0], flow_xy[:,1]]
        flow_labels2[start_xy[:,0], start_xy[:,1]] = pixel_labels_arr[flow_xy2[:,0], flow_xy2[:,1]]

        # Greedily expand the cell one pixel at a time
        greedy_expansion(start_xy, pixel_labels_arr, flow_labels, flow_labels2, foreground_mask, max_expansion_steps=max_expansion_steps)
    ...
    rows = start_xy[update_mask,0]*pixel_labels_arr.shape[1] + start_xy[update_mask,1]
    cols = flow_xy[update_mask,0]*pixel_labels_arr.shape[1] + flow_xy[update_mask,1]
    graph = csr_matrix((np.ones(len(rows)), (rows,cols)), shape=(update_mask.shape[0],update_mask.shape[0]))

    # Estimate connected subgraphs
    n_connected, connected_labels = connected_components(csgraph=graph, directed=False, return_labels=True)
    ...
    uniques, inv_map, unique_counts = np.unique(pixel_labels_arr[:class_preds.shape[0], :class_preds.shape[1]], return_inverse=True, return_counts=True)
    ...
    class_labels[uniques[1:]-1] = class_avgs[1:].argmax(axis=1) # MLE class assignment
...
nucleus_mask = labels_geo_df['nucleus_distance'] <= foreground_nucleus_distance
pixel_nuclei_arr[labels_geo_df['X'].values[nucleus_mask], labels_geo_df['Y'].values[nucleus_mask]] = labels_geo_df['nucleus_label'][nucleus_mask]

# Get the distribution of genes in each cell type
empirical_gene_probs = np.zeros((class_preds.shape[-1], n_genes))
gene_class_labels = class_labels[segments[tx_geo_df['x_location'].astype(int), tx_geo_df['y_location'].astype(int)]-1]
np.add.at(empirical_gene_probs, (gene_class_labels[gene_class_labels>=0], tx_geo_df['gene_id'][gene_class_labels>=0]), 1)
empirical_gene_probs = empirical_gene_probs / empirical_gene_probs.sum(axis=1, keepdims=True)
marker_weights = empirical_gene_probs / empirical_gene_probs.mean(axis=0, keepdims=True)
...
for idx in range(marker_weights.shape[0]):
    ...
    axarr[i,j].scatter(np.arange(n_markers)+1, marker_weights[idx,np.argsort(marker_weights[idx])[::-1]][:n_markers])
    axarr[i,j].set_xticks(np.arange(n_markers)+1)
    axarr[i,j].set_xticklabels(markers[np.argsort(marker_weights[idx])[::-1]][:n_markers], rotation='vertical')
plt.tight_layout()
...
for i, ((x_start, x_stop), (y_start, y_stop)) in enumerate(zip(x_regions, y_regions)):
    local_nuclei = np.array(pixel_nuclei_arr[x_start:x_stop,y_start:y_stop], dtype=float)
    ...
    local_celltypes[local_segments >= 0] = class_labels[local_segments[local_segments >= 0].astype(int)]
    axarr[1,i].imshow(local_foreground, vmin=0, vmax=1, cmap='coolwarm', interpolation='none')
    ...
    local_nuclei[local_nuclei >= 0] = local_nuclei[local_nuclei >= 0] + len(np.unique(local_segments))
    local_segments[local_segments >= 0] = local_segments[local_segments >= 0] + len(np.unique(local_segments))
    local_labels = np.unique(local_segments)
    if len(local_labels) > 20:
        local_labels = local_labels[local_labels >= 0]
        for j,c in enumerate(np.random.choice(local_labels, size=len(local_labels), replace=False)):
...
def save_cell_matrix(pixel_labels_arr, tx_geo_df, outfile):
    ...
    X, Y, gene_id = tx_geo_df['x_location'].values.astype(int), tx_geo_df['y_location'].astype(int), tx_geo_df['gene_id']
    ...
    x_centers, y_centers, n_pixels = np.zeros(n_cells), np.zeros(n_cells), np.zeros(n_cells)
    grid_xy = np.array(np.meshgrid(np.arange(pixel_labels_arr.shape[0]), np.arange(pixel_labels_arr.shape[1]))).T.reshape(-1,2)
    assigned = pixel_labels_arr.reshape(-1) > 0
    np.add.at(x_centers, pixel_labels_arr.reshape(-1)[assigned]-1, grid_xy[assigned,0])
    np.add.at(y_centers, pixel_labels_arr.reshape(-1)[assigned]-1, grid_xy[assigned,1])
    np.add.at(n_pixels, pixel_labels_arr.reshape(-1)[assigned]-1, 1)
    centroids = np.array([x_centers/np.clip(n_pixels,1,None), y_centers/np.clip(n_pixels,1,None), n_pixels]).T

    # Build a dataframe
    df = {'centroid_x': x_centers/np.clip(n_pixels,1,None),
